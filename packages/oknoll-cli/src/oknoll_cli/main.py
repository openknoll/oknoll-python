"""oknoll CLI — the frozen command surface.
...
def add(
    ...
    if source.startswith("cloud:"):
        _not_yet("add (cloud sources)", "the hosted control plane")
...
def chat(
    ...
    if any(ref.startswith("cloud:") for ref in bundle):
        _not_yet("chat (cloud bundles)", "the hosted control plane")
    if len(bundle) > 1:
...
def serve(
    ...
    """Serve the seven deterministic navigation tools over stdio MCP."""
    if not mcp:
...
def keys_create() -> None:
    ...
    _not_yet("keys create", "the hosted control plane")


@keys_app.command("list")
def keys_list() -> None:
    """List API keys."""
    _not_yet("keys list", "the hosted control plane")


@keys_app.command("revoke")
def keys_revoke(key_id: str = typer.Argument(...)) -> None:
    """Revoke an API key."""
    _not_yet("keys revoke", "the hosted control plane")
