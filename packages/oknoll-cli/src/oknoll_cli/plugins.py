"""`oknoll plugin` — list, inspect, validate installed connectors (design §5.1).

Only first-party connectors exist (the public connector marketplace is out of
scope, design §2.4), so "installed" means the connectors this build ships.
Validation is structural: does the class satisfy the frozen §6.2 protocol?
...
def validate(connector: type[Any]) -> list[str]:
    """Structural §6.2 conformance problems (empty = conformant)."""
    problems: list[str] = []
