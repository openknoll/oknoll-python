...
def project(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    result = runner.invoke(app, ["project", "init", str(tmp_path), "--name", "handbook"])
...
def test_full_local_flow(project: Path) -> None:
    result = runner.invoke(app, ["source", "add", "sources/handbook"])
    assert result.exit_code == 0, _output(result)
    ...
    again = runner.invoke(app, ["source", "add", "sources/handbook"])
    ...
    repack = runner.invoke(app, ["bundle", "pack"])
...
def test_semantic_diff_between_two_revisions(project: Path) -> None:
    ...
    assert runner.invoke(app, ["project", "build"]).exit_code == 0
    ...
    result = runner.invoke(app, ["bundle", "diff", rev_a, rev_b])
    assert result.exit_code == 0, _output(result)
    out = _output(result)
    ...
    implicit = runner.invoke(app, ["bundle", "diff", rev_a])
    assert implicit.exit_code == 0, _output(implicit)
    assert f"{rev_a} → {rev_b}" in _output(implicit)

    as_json = runner.invoke(app, ["bundle", "diff", rev_a, "--json"])
    ...
    same = runner.invoke(app, ["bundle", "diff", rev_b, rev_b])
...
def test_diff_argument_validation(project: Path) -> None:
    ...
    bare = runner.invoke(app, ["bundle", "diff"])
    assert bare.exit_code == 1
    assert "give a revision id" in _output(bare)

    mixed = runner.invoke(app, ["bundle", "diff", "rev-000000000abc", "--check"])
    assert mixed.exit_code == 1
    assert "--check takes no revision arguments" in _output(mixed)

    unknown = runner.invoke(app, ["bundle", "diff", "rev-000000000abc"])
...
def test_add_unsupported_source_fails(project: Path) -> None:
    ...
    result = runner.invoke(app, ["source", "add", "data.csv"])
    assert result.exit_code == 1
...
def test_add_outside_project_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.chdir(tmp_path)
    result = runner.invoke(app, ["source", "add", "whatever.md"])
    assert result.exit_code == 1
    assert "no oknoll.toml" in _output(result)


def test_generation_version_bump_regenerates_deliberately(project: Path) -> None:
    runner.invoke(app, ["source", "add", "sources/handbook"])
    first = runner.invoke(app, ["project", "build"])
    ...
    second = runner.invoke(app, ["project", "build"])
