...
def test_unknown_model_spec_fails_cleanly(empty_project: Path) -> None:
    result = runner.invoke(app, ["query", "ask", "anything?", "--model", "gpt-42"])
...
def test_anthropic_without_key_names_the_env_var(
    ...
    result = runner.invoke(app, ["query", "ask", "anything?", "--model", "anthropic:claude-opus-5"])
    assert result.exit_code == 1
    output = _output(result)
...
def test_dotenv_in_project_root_supplies_the_key(
    ...
    try:
        result = runner.invoke(
            app, ["query", "ask", "anything?", "--model", "anthropic:claude-opus-5"]
        )
        assert result.exit_code == 1
        output = _output(result)
        assert "ANTHROPIC_API_KEY" not in output
        assert "run `oknoll project build` first" in output
    finally:
