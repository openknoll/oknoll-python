"""`ask`: navigation policy (§7.1), answer contract (§7.3), and the trace record.
...
def test_navigation_policy_starts_at_overview_and_peeks_before_reading(multihop: Path) -> None:
    ...
    assert tools[0] == "overview"  # §7.2: overview is the first call
    assert tools[1] == "search"  # §7.1: narrow before reading
    assert tools.index("peek") < tools.index("read")  # §7.1: peek before read
